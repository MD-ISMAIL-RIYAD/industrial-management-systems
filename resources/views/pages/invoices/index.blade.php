    ...
    <div class="card mb-3 p-4">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered">
                <thead class="thead-dark"><tr><th>Id</th><th>Invoice number</th><th>Order id</th><th>Customer id</th><th>Invoice date</th><th>Subtotal</th><th>Tax amount</th><th>Discount amount</th><th>Total amount</th><th>Paid amount</th><th>Payment status</th><th>Status</th><th>Created at</th><th>Updated at</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($invoices as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ $item->invoice_number }}</td><td>{{ optional($item->order)->name ?? $item->order_id }}</td><td>{{ optional($item->customer)->name ?? $item->customer_id }}</td><td>{{ $item->invoice_date }}</td><td>{{ $item->subtotal }}</td><td>{{ $item->tax_amount }}</td><td>{{ $item->discount_amount }}</td><td>{{ $item->total_amount }}</td><td>{{ $item->paid_amount }}</td><td>{{ $item->payment_status }}</td><td>{{ $item->status }}</td><td>{{ $item->created_at }}</td><td>{{ $item->updated_at }}</td><td>
