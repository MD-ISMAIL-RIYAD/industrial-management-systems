    ...
    <div class="card mb-3 p-4">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered">
                <thead class="thead-dark"><tr><th>Id</th><th>Code</th><th>Name</th><th>Product id</th><th>Qty</th><th>Labour cost</th><th>Date</th><th>Remark</th><th>Actions</th></tr></thead>
                <tbody>
                @foreach ($boms as $item)
                    <tr><td>{{ $item->id }}</td><td>{{ $item->code }}</td><td>{{ $item->name }}</td><td>{{ optional($item->product)->name ?? $item->product_id }}</td><td>{{ $item->qty }}</td><td>{{ $item->labour_cost }}</td><td>{{ $item->date }}</td><td>{{ $item->remark }}</td><td>
